...
makeLemmingEvent
  :: forall a
   . ((forall b. Event b -> (b -> Effect Unit) -> ST Global (ST Global Unit)) -> (a -> Effect Unit) -> ST Global (ST Global Unit))
  -> Event a
makeLemmingEvent e = Event $ mkSTFn2 \tf k -> do
  let
    o :: forall aa. Event aa -> (aa -> Effect Unit) -> ST Global (ST Global Unit)
    o (Event ev) kx = runSTFn2 ev tf (mkEffectFn1 kx)

  (e o) (\a -> runEffectFn1 k a)

newtype Subscriber = Subscriber (forall b. STFn2 (Event b) (EffectFn1 b Unit) Global (ST Global Unit))

makeLemmingEventO
  :: forall a
   . STFn2 Subscriber (EffectFn1 a Unit) Global (ST Global Unit)
  -> Event a
makeLemmingEventO e = Event $ mkSTFn2 \tf k -> do
  let

    o :: forall aa. STFn2 (Event aa) (EffectFn1 aa Unit) Global (ST Global Unit)
    o = mkSTFn2 \(Event ev) kx -> runSTFn2 ev tf kx

  runSTFn2 e (Subscriber o) k
...
makeEvent
  ...
  -> Event a
makeEvent e = Event $ mkSTFn2 \tf k -> do
...
-- bus f = Event $ mkSTFn2 \_ k -> do
--   { push, event } <- create
--   runEffectFn1 k (f push event)
--   pure (pure unit)
-- pusher <- doOnce (runEffectFn1 k (f push event)) (\x -> runEffectFn1 push x)
-- runSTFn2 e b $ mkEffectFn1 pusher

doOnce :: forall f a. Functor f => Effect Unit -> (f (Effect a)) -> ST Global (f (Effect a))
doOnce e f = do
  triggered <- STRef.new false
  pure $ f <#> \x -> do
    t <- liftST $ STRef.read triggered
    void $ liftST $ STRef.write true triggered
    when (not t) do
      e
    x
...
memoize (Event e) f = Event $ mkSTFn2 \b k -> do
  { push, event } <- create'
  -- runEffectFn1 k (f event)
  -- runSTFn2 e b push
  pusher <- doOnce (runEffectFn1 k (f event)) (\x -> runEffectFn1 push x)
  runSTFn2 e b $ mkEffectFn1 pusher
...
--   r <- STRef.new i
--   { event, push } <- create'
--   unsubscribe <- runSTFn2 e true $ mkEffectFn1 \x -> do
--     void $ liftST $ STRef.write x r
--     runEffectFn1 push x
--   pure
--     { event: event <|>
--         ( Event $ mkSTFn2 \_ k -> do
