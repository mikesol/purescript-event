module FRP.Event.EffectFn.VBus (V, vbus, class VBus, VbusT, Vbus(..), vb, vbackdoor, VBackdoor) where
...
import FRP.Event.EffectFn (Event, create, makeEvent)
...
instance vbusCons1 ::
  ...
  vb _ = do
    p /\ e <- vb (Proxy :: _ rest)
    { event, push } <- create
    pure (Record.insert (Proxy :: _ key) (push :: z -> Effect Unit) p /\ Record.insert (Proxy :: _ key) event e)
